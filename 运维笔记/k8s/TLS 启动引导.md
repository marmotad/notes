...
向 kube-apiserver 添加 `--token-auth-file=FILENAME` 标志（或许这要对 systemd 的单元文件作修改）以启用令牌文件。参见 [这里](https://kubernetes.io/zh/docs/reference/access-authn-authz/authentication/#static-token-file) 的文档以了解进一步的细节。
